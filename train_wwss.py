...
def get_cfg(args):
    ...
    # retype
    assert cfg.retype in ['all', 'unlabel']
...
def train(label_loader, unlabel_loader, model, model2, criterion, optimizer, epoch, logger, writer, cfg):
    ...
    for batch_idx in range(num_iters):
        # fetch data
        try:
            (inputs_x_w1, inputs_x_w2, inputs_x_s1, inputs_x_s2), targets_x, w_x = next(labeled_train_iter)
        except:
            assert False
        try:
        ...
        # to cuda
        inputs_x_w1, inputs_x_w2, inputs_x_s1, inputs_x_s2 = \
            inputs_x_w1.cuda(), inputs_x_w2.cuda(), inputs_x_s1.cuda(), inputs_x_s2.cuda()
        inputs_u_w1, inputs_u_w2, inputs_u_s1, inputs_u_s2 = \
            inputs_u_w1.cuda(), inputs_u_w2.cuda(), inputs_u_s1.cuda(), inputs_u_s2.cuda()
        ...
        with torch.no_grad():
            ...
            outputs_u_w21 = model(inputs_u_w2)
            outputs_u_w12 = model2(inputs_u_w1)
            outputs_u_w22 = model2(inputs_u_w2)

            pu = (torch.softmax(outputs_u_w11, dim=1) + torch.softmax(outputs_u_w21, dim=1) +
                  torch.softmax(outputs_u_w12, dim=1) + torch.softmax(outputs_u_w22, dim=1)) / 4
